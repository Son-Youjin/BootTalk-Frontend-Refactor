...
기존 프로젝트를 다시 처음부터 살펴보면서 단순히 코드를 정리하는 것과 실제로 유지보수하기 좋은 코드를 만드는 것은 다르다는 것을 느꼈습니다.

처음에는 단순히 모바일 UI를 개선하고 컴포넌트를 정리하는 데 집중했지만, 작업을 진행하면서 필터링이나 무한 스크롤처럼 기존에 정상적으로 동작한다고 생각했던 기능에서도 여러 문제를 발견했습니다.  
특히, 필터 조건 변경에 따른 데이터 갱신 문제와 무한 스크롤의 API 과다 호출 문제를 해결하면서, 화면의 결과만 보는 것이 아니라 `queryKey`, 캐싱, 페이지네이션 상태와 같은 데이터 흐름까지 확인해야 문제의 원인을 정확하게 찾을 수 있다는 것을 배웠습니다.

또한 기존 컴포넌트를 기능별로 분리하고 반복되는 UI를 재사용할 수 있도록 개선하면서, 처음부터 컴포넌트의 책임을 명확하게 나누는 것이 이후의 수정과 확장에 큰 영향을 준다는 것도 경험하게 되었습니다.

<br/>
이번 프로젝트에서는 단순히 "동작하는 코드"를 만드는 것에서 한 단계 더 나아가, **왜 문제가 발생했는지 파악하고 유지보수하기 좋은 구조로 개선하는 과정**을 경험할 수 있었습니다.

앞으로는 새로운 기능을 구현할 때도 일단 동작하게 만드는 것에 그치지 않고, 상태와 데이터의 흐름, 컴포넌트의 책임, 성능까지 함께 고려하면서 개발하는 것을 목표로 하고 있습니다.
